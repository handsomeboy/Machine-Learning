...
def getRecall(labels,predictedLabels, positive_label):
    if (getTP(labels,predictedLabels,positive_label) == 0):
        ...
        recall = getTP(labels,predictedLabels,positive_label)/float(getTP(labels,predictedLabels,positive_label)+getFN(labels,predictedLabels,positive_label))
...
def getPrecision(labels,predictedLabels, positive_label):
    if(getTP(labels,predictedLabels,positive_label) == 0):
        ...
        precision = getTP(labels,predictedLabels,positive_label)/float((getTP(labels,predictedLabels,positive_label)+getFP(labels,predictedLabels,positive_label)))
