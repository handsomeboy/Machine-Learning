...
def main():
    ...
    for lw in [x * 0.00001 for x in range(1, 11)]:
        ...
        thetas, iterations, errors = gradient_descent(z,data_Y, learning_weight=lw, threshold=10)
        print("Itearative Method Coefficients: {},Iterations:{}".format(thetas,iterations))
        #evaluate performance
        performance = kfold_validation_gradient_descent(z,data_Y,10, lw,threshold=10)
        values = np.append(values,[[lw,iterations,performance]],axis=0)

    fig, ax1 = plt.subplots()
    ax1.set_xlabel('Learning Weight')
    ...
    ax2 = ax1.twinx()
    ax2.plot(values[:,0],values[:,2],color='red')
